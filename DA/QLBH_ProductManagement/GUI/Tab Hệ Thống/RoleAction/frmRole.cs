...
namespace QLBH_ProductManagement.GUI
{
    public partial class frmPhanQuyen : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void treeGroupUser_RowCellClick(object sender, DevExpress.XtraTreeList.RowCellClickEventArgs e)
        {
            ...
            List<GroupUser> listGroup = grUserBUS.getGroupUserFromName(namePraNode);  // lấy ra được thông tin của nhóm user
           
            listUser = user_BUS.GetUserGroup(listGroup[0].GR_ID, listGroup[0].Group_Name); // lấy ra danh sach người dùng thuộc nhóm
        ...
        private void gridViewGroupUser_MasterRowEmpty(object sender, DevExpress.XtraGrid.Views.Grid.MasterRowEmptyEventArgs e)
        {
            ...
            GroupUser grUser = grv.GetRow(e.RowHandle) as GroupUser;
            if (grUser != null)
            {
                e.IsEmpty = !listUser.Any(x => x.groupID == grUser.GR_ID);
            }
        }
